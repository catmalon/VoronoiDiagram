...
class VoronoiDiagram:
	...
	@staticmethod
	def voronoi_of(points, debug=None):
		if (len(points) == 1):
			return VoronoiDiagram(points, [], ConvexHull.convexhull_of(points))
		elif (len(points) == 2):
			divider = Divider.divider_of(points[0],points[1])
			convex = ConvexHull.convexhull_of(points)
			return VoronoiDiagram(points, [divider], convex)
		elif (len(points) == 3):
			if (Triangle.calc_area(points[0], points[1], points[2]) == 0):
				points.sort(key=lambda p: p.x)
				d1 = Divider.divider_of(points[0], points[1])
				d2 = Divider.divider_of(points[1], points[2])
				convex = ConvexHull.convexhull_of(points)
				return VoronoiDiagram(points, [d1, d2], convex)
			else:
				triEx = Triangle.circumcenter_of(points[0], points[1], points[2])
				Point.sort_counterclockwisely(points)
				dividers = []
				for i in range(len(points)):
					d1 = Divider.divider_of(points[i], points[(i+1)%3])
					n1 = Vector.vector_of(points[i], points[(i+1)%3])
					v1 = Vector.normalvector_of(n1)
					
					d1.start = Point(triEx.x + v1.dx*600, triEx.y + v1.dy*600)
					d1.end = triEx
					if (d1.start.y > d1.end.y):
						d1.start, d1.end = d1.end, d1.start
					dividers.append(d1)
		...
		vleft = VoronoiDiagram.voronoi_of(points[:half])
		vright = VoronoiDiagram.voronoi_of(points[half:])
